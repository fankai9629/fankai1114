          ...
          <router-link :to="{name:'filmDetail',params:{id:item.filmId}}">
            ...
            <div class="film-list-item-desc">
              ...
              <p class="item-desc-times info-col">
                <label>上映时间：</label> 
                <span>{{item.premiereAt|getnewDay}}</span>
              </p>
            </div>
            <div class="film-list-item-action padding" v-if="item.isPresale">
              <span  class="buy">预购</span>
            </div>
          </router-link>
        </li>
      </ul>
...
import { getFilmComming } from "@/api/film.js";
...
export default {
  ...
  methods: {
    getData() {
      getFilmComming({ pageNum: this.pageInfo.pageNum }).then(result => {
        if (result.status == 0) {
            console.log(result)
